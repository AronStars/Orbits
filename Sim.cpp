...
OrbitSimulator::OrbitSimulator(): dispvelx(0), dispvely(0), selectedIndex(-1), isDragging(false),
    ...
    bodies.emplace_back(pos1, vel1, mass, 8.f);
    bodies.emplace_back(pos2, vel2, mass, 8.f);
    bodies.emplace_back(pos3, vel3, mass, 8.f);
...
void OrbitSimulator::draw() {
    ...
    if (selectedIndex >= 0 && selectedIndex < static_cast<int>(bodies.size())) {
        ...
        DrawText(info, screenWidth - 280, 70, 20, LIGHTGRAY);
